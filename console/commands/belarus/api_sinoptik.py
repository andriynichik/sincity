...
for row in db.belarus.find({}, no_cursor_timeout=True):
    ...
    if not 'sinoptik_db_id' in row :
        ...
        if str(row['SOATO']) in data_isset:
            ...
            print('yessss', data_isset[str(row['SOATO'])])
        else:
            try:
                types_list = ["Область","Район области","Сельсовет"]
 
                if not row['NAMEREGION'] in types_list:
                    if 'status_snig' in row or 'status_ins' in row:
