...
class Loader:
    ...
    def loader_gmaps(gmaps_config):
        return LoaderGMapsWOCache(googlemaps=googlemaps.Client(key=gmaps_config.get('geocoding').get('key')))
    ...
    def loader_gmaps_with_cache(gmaps_config, storage_config):
        ...
        return LoaderGMapsWithCache(googlemaps=googlemaps.Client(key=gmaps_config.get('geocoding').get('key')), storage=storage)
